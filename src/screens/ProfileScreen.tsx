...
export function ProfileScreen() {
  ...
  const getUserId = async (): Promise<string | undefined> => {
    try {
      ...
      if (authError) {
        console.warn(
          "auth.getUser error (fallback to table):",
      ...
      console.warn(
        "auth.getUser threw, will fallback to selecting first profile id",
        e
      );
    }
  ...
  // Load all user data in one useEffect
  useEffect(() => {
    const loadUserData = async () => {
      ...
      // Load other data
      const [completed, points, name, level] = await Promise.all([
        getUserCompleted(),
        getUserPoints(),
        getUserName(),
        getUserLevel(),
  ...
  const stats = [
    ...
    {
      label: "Ukupno poena",
      value: Number(userPoints).toLocaleString(),
      icon: Award,
    },
  ];
        ...
        <div className="profile-achievements-grid">
          {achievements.map((achievement, index) => (
            ...
            >
              ...
              <p
                className={`profile-achievement-description ${
                  achievement.unlocked ? "unlocked" : "locked"
                }`}
              >
